...
며칠 전, 윈도우 애저의 무료 사용량을 초과해서 서비스가 끊어졌다. 당분간은 서비스 할 수 없을 것 같다.

● 2012년 10월 4일.

프로젝트를 더 개선시켜보려 한다. 누군가가 만든 너무 멋진 웹 체스를 보고 말아서... http://en.lichess.org/

아직 안써본 jQuery UI도 써보고 싶고, DB도 써보고 싶고, 이 애플리케이션을 아예 다트나 TypeScript같은 언어로 이전시켜볼까도 고려 중이다.

● 2012년 10월 3일.

전역 변수들에 대해서 네임스페이스 패턴을 적용했다.

● 2012년 10월 2일.

쓰리폴드 레피테이션(같은 포지션이 3번 반복되면 무승부)이라는 룰을 추가하고, 몇몇 버그들을 잡았다. 
...
체크메이트를 제대로 감지하지 못하던 버그를 수정했다. 버그가 얼마나 더 숨어있을지 ㅠㅠ

터치 이벤트를 마우스 이벤트로 바인드하는 부분을 전면 개선했다. 이전에는 터치 이벤트가 발생하면 마우스 이벤트를 강제로 생성, 초기화해서 작동시켰었는데, 이것 때문에 터치 이벤트가 필요한 다른 엘리먼트에 터치 이벤트를 사용할 수 없는 문제가 있었다. (예를 들어 채팅창 input 엘리먼트) 이제는 터치 이벤트도 상응하는 마우스 이벤트와 같은 콜백 함수를 리스너에 등록하고, 각 콜백 함수들은 가장 먼저 아래의 bindEvent()를 호출하여 터치 이벤트도 마우스 이벤트와 동등하게 취급한다.
...
이러면 터치 이벤트를 사용하면서도 드래그하는데 문제가 없다. 이제 서버 로직도 문제 없어보이고, 발견된 주요 문제들은 모두 해결한 것 같으니 이 프로젝트는 완료한 것으로 하고 가끔 버그 수정, 리팩토링 정도만 하면 될 것 같다.

● 2012년 9월 28일.

레이아웃을 확실하게 잡았다. 이제 모든 브라우저에서 똑같이 보인다. 그래도 뭔가 욕심이 생겨 반응형 웹 책 한권을 주문해놨다.

게임 중간에 들어오는 게스트에게 제공할 체스판 정보를 담고 있던 전역 범위의 배열을 삭제했다. 지금까진 클라이언트의 onbeforeunload 이벤트 등을 사용해서 접속 끊는 것을 확인하여 빈 배열을 삭제해왔는데, 일부 모바일 브라우저 등이 onbeforeunload 이벤트를 지원하지 않아서 disconnect를 감지하지 못해 영구적으로 배열이 삭제되지 않는 문제가 있었다. 이젠 체스판 정보를 서버에게 제공받지 않고 White 유저로 부터 제공받게 된다.

이제 방장(화이트)이 접속을 종료하면 해당 방의 나머지 모든 클라이언트가 에러 페이지로 이동한다. 

게스트들은 이제 고유 이름을 가진다. socket.id의 0에서 3번째 문자열을 substring 한 것을 이름으로 한다. 

클라이언트가 입장/퇴장할 때, 방의 인원 수가 표시된다.

● 2012년 9월 27일.

룰 유효성 검사 코드를 리팩토링했다. 중복되는 코드를 줄이고 줄여서 원래 1200라인이 넘었던 것이 600라인 밑으로 내려갔다 헐~~.. 이 과정에서 버그도 많이 잡았다. 그러나 그 과정에서 또 버그가 생겼을 지도 모르는 일 ㅜㅠ
